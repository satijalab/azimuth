...
library(shinyjs)
library(shiny)
load_all("/home/haoy/seurat-disk/")
load_all("/home/haoy/uwot/")
load_all("/home/haoy/sctransform/")
load_all("/home/haoy/seurat-private/")
options(shiny.maxRequestSize = 100 * (1024 ^ 2))
# example app for prepending/appending a navbarMenu
reference <- readRDS("/home/haoy/fast-mapping/pbmc_ref.diet.rds")
reference.nn <- readAnnoyNN("/home/haoy/fast-mapping/pbmc_ref.nn.rds")

ui <- tagList(
  useShinyjs(),
  fluidPage(
    title = "SuperCoolMappingNameTBD",
    sidebarLayout(
    ...
    handlerExpr = {
      withProgress(
        ...
        expr = {
          ...
          suppressWarnings(object <- SCTransform(
            object,
            residual.features  = rownames(x = reference),
            ncells = 3000
          ))
          setProgress(value = 1,message = 'Finished pre-processing')
          output$qcViolin <- shiny::renderPlot(expr = Seurat::VlnPlot(plot.env$user,'nFeature_RNA'))
          ...
          insertTab(
            ...
            tabPanel(
              title = "QC",
              "We should add a table of QC stats here",
    ...
    handlerExpr = {
      withProgress(
        ...
        expr = {
          ...
          spca.anchor <- FindTransferAnchors_Fast(
            reference = reference,
            query = plot.env$user,
          ...
          ingest.spca <- IngestNewData_Fast(
            reference = reference,
            query = plot.env$user,
            dims = 1:50,
          ...
          plot.env$user[["umap.proj"]] <- RunUMAP(
            object = ingest.spca[["query_ref.nn"]],
            reduction.model =  reference[["jumap"]],
            reduction.key = "ProjU_"
          )
          output$mapDimPlot <- shiny::renderPlot(expr = Seurat::DimPlot(
            plot.env$user,
            reduction = 'umap.proj',
          ...
          output$metadata <- downloadHandler(
            filename =  function() {
              return(paste0(input$name, "_metadata.csv"))
            },
            content = function(file){
              ...
              sidebarLayout(
                sidebarPanel(
                  ...
                  selectInput(
                    inputId = 'feature',
                    label = 'Gene',
                    choices = c(sort(features),'predicted.id.score'),
                    selected = "GNLY",
                    selectize = FALSE,
                    width = '100%'
                  )
