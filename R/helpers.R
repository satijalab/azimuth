#' Transfer downloaded Azimuth results into Seurat object (or create a new one)
#'
#' @param object existing Seurat object to add azimuth_results to (creates new object if not given)
#' @param azimuth_results result of downloading "All results" from Azimuth,
#' or path to file containing these results
#'
#' @importFrom Seurat CreateSeuratObject AddMetaData
#'
#' @export
AzimuthToSeurat <- function(object = NULL, azimuth_results = NULL) {
  if (is.null(x = azimuth_results)) {
    stop("No Azimuth results provided.")
  }
  if (is.character(x = azimuth_results)) {
    message(paste0("Reading from file ", azimuth_results))
    azimuth_results <- readRDS(file = azimuth_results)
  }
  ...
  if (is.null(x = object)) {
    ...
    object <- CreateSeuratObject(
      counts = matrix(
        nrow = 1,
        ncol = nrow(azimuth_results$umap@cell.embeddings),
        dimnames = list(
    ...
    overlap.cells <- intersect(
      Cells(object),
      rownames(azimuth_results$umap@cell.embeddings)
    )
    if (length(x = overlap.cells) < 0.05 * length(x = Cells(object))) {
      stop("Cells in object do not match cells in download")
    } else if (length(x = overlap.cells) < length(x = Cells(object))) {
      message(paste0("Subsetting out ", length(x = Cells(object)) - length(x = overlap.cells),
                     " cells that are absent in downloaded results (perhaps filtered)"))
