...
RunAzimuth.Seurat <- function(
  ...
  bridge <- FALSE
  if (class(query[[assay]]) == "ChromatinAssay") {
    bridge <- TRUE
  }
  if (dir.exists(reference)) {
    # make sure the bridge extension is contained in the directory
    if (bridge) {
      if (!file.exists(file.path(reference, "bridge.Rds"))) {
        stop(paste("bridge.Rds not found in the reference directory:", reference))
      } else {
        bridge.extension <- readRDS(file.path(reference, "bridge.Rds"))
      }
    }
  ...
  if (bridge) {
    bridge.anchor <- FindBridgeTransferAnchors(
      extended.reference = bridge.extension,
      query = query,
      query.assay = assay,
